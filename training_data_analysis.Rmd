...
json <- Sys.getenv("SHEETS_API_JSON")

gs4_auth_configure(gs4_auth(email = "leahmhkemp@gmail.com", path=json))

# read in data from google sheets
M5_GYM_FULL_BODY <- read_sheet(training_plan_july_2021_july_2022,
                               sheet = "M5_GYM_FULL_BODY")

M5_DAILY_FLEX <- read_sheet(training_plan_july_2021_july_2022,
                            sheet = "M5_DAILY_FLEX")

# M4_POLE_CONDITIONING <- read_sheet(training_plan_july_2021_july_2022,
#                                    sheet = "M4_POLE_CONDITIONING")
M4_DAILY_FLEX <- read_sheet(training_plan_july_2021_july_2022,
                            sheet = "M4_DAILY_FLEX")

M4_POLE_OPEN_TRAIN <- read_sheet(training_plan_july_2021_july_2022,
                                 sheet = "M4_POLE_OPEN_TRAIN")

M4_GYM_FULL_BODY <- read_sheet(training_plan_july_2021_july_2022,
                               sheet = "M4_GYM_FULL_BODY")

M3_DAILY_FLEX <- read_sheet(training_plan_july_2021_july_2022,
                            sheet = "M3_DAILY_FLEX")

M3_POLE_UPPER_BODY_CORE <- read_sheet(training_plan_july_2021_july_2022,
                                sheet = "M3_POLE_UPPER_BODY_CORE")

M3_LOWER_BODY <- read_sheet(training_plan_july_2021_july_2022,
                               sheet = "M3_LOWER_BODY")
...
dataset_list <- list(M5_GYM_FULL_BODY,
                     M5_DAILY_FLEX,
                     M4_DAILY_FLEX,
                     M4_POLE_OPEN_TRAIN,
                     M4_GYM_FULL_BODY,
...
prepare_weights_data <- function(df_list) {
  ...
  data <- df_list %>%
    ...
    mutate_at(vars(matches("\\d{1,2}/\\d{1,2}/\\d{1,4}.set")), as.double) %>%
...
weight_reps_pairs <- data %>%
  ...
  group_by(exercise,reps_or_secs, weight) %>%
  summarise(reps_or_secs, weight)
...
### COLOURING OF POINTS NOT WORKING ###

# pair each exercise with a colour so they are consistent between all the plots
colours_list <- data.frame(colour = paste0(rainbow(n = length(exercises))),
                           exercise = exercises)

colours_list$colour = paste0('"', colours_list$colour, '"')

# join my column column back into data
data <- left_join(data, colours_list, by = "exercise")

# setup ui
ui <- fluidPage(
  
  # generate box for user to select exercise
  selectizeInput(
    inputId = "exercises", 
    label = NULL,
    choices = c("Choose an exercise" = "", exercises), 
    multiple = TRUE
  ),
...
server <- function(input, output, session, ...) {
  ...
  # render scatterplot based on current user exercise choice
  output$scatterplot <- renderPlotly({
    
    req(input$exercises)
    
    # get current screen width of
    cur_height <- session$clientData$output_p_height
    cur_width <- session$clientData$output_p_width
    
    # plot
    scatterplot <- plot_ly(data = filter(data, exercise %in% input$exercises),
                           x = ~date,
                           y = ~weight,
                           type = "scatter",
                           mode = "markers",
                           hoverinfo = "text",
                           text = ~ base::paste("</br><i>", exercise, "</i>",
                                                "</br><br> Weight:", weight,
                                                "</br> Reps/secs:", reps_or_secs,
                                                "</br> Date:", date),
                           marker = base::list(colors = ~colour),
                           size = ~reps_or_secs) %>%
      plotly::layout(width = cur_width,
                     height = cur_height,
                     xaxis = base::list(title = "Date"),
                     yaxis = base::list(title = "Weight (kg)"))
...
```{r, out.width = "100%"}
DAILY_FLEX <- full_join(M3_DAILY_FLEX, M4_DAILY_FLEX,  by = c("requirement", "exercise"))

# remove microbend drills
DAILY_FLEX <- DAILY_FLEX %>%
  filter(requirement != "Microbend drills")
...
# make this data long for downstream analyses
# this also extracts the date from the column names and makes a variable out of it
DAILY_FLEX_long <- pivot_longer(DAILY_FLEX,
...
# format date column as a date
DAILY_FLEX_long$date <- as.Date(DAILY_FLEX_long$date,"%d/%m/%y")
